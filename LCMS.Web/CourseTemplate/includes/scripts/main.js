//CURRENT IN TRADITIONAL CHINESE
$(document).ready(function(){
	...
	for(var module = 0; module < TOC.length; module++){
		...
		//console.log(courseType.indexOf("shortcourse"));
		
		//show the proper number of module links in the header...
		if(courseType.indexOf("shortcourse") === -1 || portalSetup.course_locale.indexOf("zh") === -1){
...
document.getElementById("main-nav-check").onclick = function(){
    ...
    if(!document.getElementById("menu")){
		...
		if(courseType.indexOf("shortcourse") === -1){								   
			...
			if(courseType.indexOf("hioc") === -1){menuHTML += "<li class=\"getStartedlink\">" + portalSetup.Labels.gettingStarted + "</li>";}//Getting Started
...
function doAlert(alertType){
	...
	switch(alertType){
		...
		case "notconnected":
			...
			bAlertBtn =  course_settings.Portal_Setup.Alerts.LRSError.Btn; //portalSetup.Labels.bOK;
	...
	if(document.getElementsByClassName("alertBtn")[0]){
		document.getElementsByClassName("alertBtn")[0].onclick = function(){
			...
			if(alertType !== "notconnected"){
				...
				//WORKS IN ALL BROWSERS EXCEPT IE11...UGH
				//var theWholeEmail = "mailto:ReportThisIssue@loma.org?subject=" + portalSetup.Course_Name + " - Error " + theLRSerror.status + ": " + theLRSerror.responseText + "&body=---" + course_settings.Portal_Setup.Alerts.LRSError.emailTellMore + "---%0D%0A%0D%0A %0D%0A%0D%0A %0D%0A%0D%0A %0D%0A%0D%0A %0D%0A%0D%0A ---------------------------------------------------------------------------------------------------------%0D%0A---" + course_settings.Portal_Setup.Alerts.LRSError.emailDoNotAlter + "---%0D%0A---" + course_settings.Portal_Setup.Alerts.LRSError.emailWeNeedThisInfo + "---%0D%0A %0D%0A" + theLRSerror.status + ": " + theLRSerror.responseText + "%0D%0A%0D%0A" + lrsActor + "%0D%0A%0D%0A" + JSON.stringify(studentBrowserInfo) + "%0D%0A%0D%0A";
...
function showWelcome(){
	...
	if(courseType.indexOf("shortcourse") !== -1){
		...
		if(courseType.indexOf("hioc") === -1){portalContents += "<div class=\"indexBtn getStartedlink\"><img src=\"includes/images/glyphicons-45-keys.png\" alt=\"Getting Started\"/>" + portalSetup.Labels.gettingStarted + "</div>";}//GETTING STARTED
...
function showDash(){
	...
	$(".dashgroup").on("click", function(){
		if(courseType.indexOf("hybrid") !== -1){
			var thisModule = this.childNodes[1].innerHTML.substring(this.childNodes[1].innerHTML.length-1);
		...
		if(this.id === "big_circle"){
			...
			for(var i = 0; i < tempLessonArrayCount; i++){
				...
				if(portalSetup.course_locale.indexOf("zh") === -1){
					...
					//lessonTemplate += "<div class=\"dashlessonnumber\">第" + chNumbers[i] + portalSetup.Labels.lesson + "</div>";//Lesson-----------------------------------------------------
			...
			$(".dashgroup").on("click", function(){
				if(courseType.indexOf("hioc") !== -1){
					...
					//console.log("this.childNodes[1].innerText = " + this.childNodes[1].innerText);
					//console.log("portalSetup.Labels.exam = " + portalSetup.Labels.exam);
...
function Mod_number(mNumber){
	...
	//}else if(courseType === "hybrid"){
		...
		if(isIOSFF === false){
			...
			moduleOutput += "<a class=\"module_btn pdf_btn\" id=\"pdf_btn\" href=\"ebooks/" + course_settings.CourseContentContent.pdflinks[mNumber].link +"\" download>" + portalSetup.Labels.module + (mNumber + 1) + " PDF</a>"; // Module...PDF -- Just a regular link unless someone requests otherwise
		}else{
		...
		if(courseType.indexOf("demo") === -1){
			if(moduleScores[mNumber][1] === "---- - -- - --"){
				// Incomplete
				moduleOutput += "<div class=\"module_btn\" id=\"exam_btn\">" + portalSetup.Labels.module + (mNumber + 1) + portalSetup.Labels.exam + "</div>"; //Module...Exam
			}else{
				// Complete
				moduleOutput += "<div class=\"module_btn_disabled\" id=\"exam_btn\">" + portalSetup.Labels.module + (mNumber + 1) + portalSetup.Labels.exam + "</div>";//Module...Exam
			}
		}else{
			moduleOutput += "<div class=\"module_btn\" id=\"exam_btn\">" + portalSetup.Labels.module + (mNumber + 1) + portalSetup.Labels.exam + "</div>"; //Module...Exam
		}
	...
	if(courseType.indexOf("hioc") !== -1){
		...
		//console.log(document.getElementsByClassName("lessonBlock")[document.getElementsByClassName("lessonBlock").length -1].offsetHeight);
	}
	...
	$("#ebook_btn").on("click", function(e){
		...
		/* if(studentBrowserInfo.device.type === "mobile" && studentBrowserInfo.browser.name.indexOf("Firefox") !== -1 ){
	...
	$(".eocbtn").on("click", function(e){
		if (e.target.id === "eocnext"){
			if(portalSetup.Course_Name.indexOf("SRI ") !== -1){
				window.open("https://www.loma.org/ProfDev/Designations/FSRI.aspx#Courses","WhatsNext", "fullscreen=yes");
			}else{
				switch(portalSetup.course_locale){
					case "en-US": window.open("https://www.loma.org/ProfDev/Designations/FLMI.aspx#Courses","WhatsNext", "fullscreen=yes"); break;
					case "zh-CN": window.open("https://www.loma.org/International/SC/ProfDev/Courses.aspx","WhatsNext", "fullscreen=yes"); break;
					case "zh-TW": window.open("https://www.loma.org/International/TC/ProfDev/Courses.aspx","WhatsNext", "fullscreen=yes"); break;

					default: window.open("https://www.loma.org/ProfDev/Designations/FLMI.aspx#Courses","WhatsNext", "fullscreen=yes"); break; // English is the default...
				}
			...
			if(document.getElementById("exam_btn").className === "hiocexam_btn"){
				...
				if(document.referrer.indexOf(COMPANY) !== -1 || document.referrer.indexOf(SCORMCLOUDLMS) !== -1){
					...
					if(document.referrer.indexOf("file:") !== -1 || document.referrer.indexOf(PRODLMS) === -1 || document.referrer.indexOf(SCORMCLOUDLMS) !== -1){ 
						var eomFile = "data=\"exam_ALL_CR=1/" + portalSetup.Course_Folder + "_EOM_" + module + ".html\""; // TESTING EOLs

					//On production	
					}else{
						var eomFile = "data=\"exam/" + portalSetup.Course_Folder + "_EOM_" + module + ".html\""; // FINAL EOLs
					}
				...
				//Original line replaced with above compound if statement...
				//var eomFile = "data=\"exam/" + portalSetup.Course_Folder + "_EOM_" + module + ".html\"";
...
function showExamPrep(){
    ...
    for(var subHead = 0; subHead < course_settings.Exam_Prep.Sections.length; subHead++){
		...
		if (course_settings.Exam_Prep.Sections[subHead].title === course_settings.Exam_Prep.indexOFLabel.PracticeQuestions){
			prepContent += "<div style=\"text-align:left;\">" + portalSetup.Labels.module + course_settings.Exam_Prep.Sections[subHead].buttons[0].module + "</div>";
